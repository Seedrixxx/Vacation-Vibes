...
export const metadata = {
  ...
  description: "Multi-destination tour packages. Explore the world with curated itineraries.",
};
...
export default async function TourPackagesPage() {
  const packages = await prisma.package.findMany({
    where: { tripType: "OUTBOUND", isPublished: true },
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {packages.length === 0 ? (
            <p className="col-span-full text-center text-charcoal/60 py-12">
              No tour packages yet. Check back soon.
            </p>
          ) : (
            packages.map((pkg) => {
